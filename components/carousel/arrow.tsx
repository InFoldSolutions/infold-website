...
export default function Arrow({ direction, clickFunction, visible = true }: { direction: string, clickFunction: any, visible: boolean }) {
  ...
  return (
    <div
      className={ `${direction} ${direction === 'left' ? '-left-[41px]' : '-right-[35px]'} ${!visible ? 'hidden' : ''} arrow absolute z-9 top-1/2 -mt-[18px] w-7 h-9 p-1 rounded-full border-2 border-neutral-200 dark:border-gray-800 dark:border-opacity-80 flex items-center justify-center cursor-pointer` }
