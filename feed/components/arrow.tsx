...
export default function Arrow({ direction, clickFunction, visible = true }: { direction: string, clickFunction: any, visible: boolean }) {
  ...
  return (
    <div
      className={ `${direction} ${direction === 'left' ? '-left-[40px]' : '-right-[40px]'} ${!visible ? 'hidden' : ''} arrow absolute z-9 top-1/2 -mt-[18px] w-7 h-9 p-1 rounded-full border-2 border-neutral-100 dark:border-neutral-600 flex items-center justify-center cursor-pointer` }
