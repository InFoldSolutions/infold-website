...
export default function Arrow({ direction, clickFunction, visible = true }: { direction: string, clickFunction: any, visible: boolean }) {
  ...
  return (
    <div
      className={ `${direction} ${direction === 'left' ? '-left-[43px]' : '-right-[43px]'} ${!visible ? 'hidden' : ''} arrow absolute z-10 top-1/2 -mt-[17.5px] w-7 h-9 p-1 rounded-full border-2 border-neutral-100 dark:border-neutral-600 flex items-center justify-center cursor-pointer` }
